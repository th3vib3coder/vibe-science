...
function gateFailureStrength(db, projectPath, gateId) {
  ...
  // Note: COUNT(*) always returns a row in SQLite, so completedCount is never null.
  const completedCount = db.prepare(`
    SELECT COUNT(*) AS cnt FROM sessions
    WHERE project_path = ? AND ended_at IS NOT NULL
  `).get(projectPath);

  if (completedCount.cnt >= COOLDOWN_SESSIONS) {
...
function observerAlertStrength(db, projectPath, messagePattern) {
  ...
  // Cooldown: if no matching alert exists in the last COOLDOWN_DAYS days,
  // the pattern has faded — suppress the hint.

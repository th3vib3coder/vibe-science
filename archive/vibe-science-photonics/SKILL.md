...
metadata:
    version: "5.5.0"
    codename: "ORO"
    skill-author: vibe-science-community
    architecture: OTAE-Tree (Observe-Think-Act-Evaluate inside Tree Search)
    lineage: "v3.5 TERTIUM DATUR → v4.0 ARBOR VITAE → v4.5 ARBOR VITAE (Pruned) → v5.0 IUDEX → v5.5 ORO"
    sources: Ralph, GSD, BMAD, Codex unrolled loop, Anthropic research, ChatGPT Spec Kit, Sakana AI-Scientist-v2 (arXiv:2504.08066v1)
    changelog: "v4.0.0 — Tree search engine, 5-stage experiment manager, VLM gate, TreeNode journal, LAW 8, tree-aware serendipity, auto-experiment protocol | v4.5.0 — Inversion+Collision brainstorm techniques, R2 red flag checklist, counter-evidence search, DOI verification, progressive disclosure refactor | v5.0.0 — Seeded Fault Injection, Judge Agent (R3), Blind-First Pass, Schema-Validated Gates. 29 gates (25 base + 4 HE; V0, J0 new in v5.0). 8 gates schema-enforced. Circuit Breaker. Agent Permission Model. Confidence formula revised. R2 structurally unbypassable. | v5.5.0 — ORO (Observe-Recall-Operate). 7 new gates (DQ1-DQ4, DC0, DD0, L-1) for data quality and operational integrity. Total: 36 gates (29 base + 4 HE + 7 v5.5 instruction-enforced; 8 schema-enforced). R2 INLINE mode (7th activation). Structured logbook (LOGBOOK.md mandatory in CRYSTALLIZE). Literature Pre-Check (L-1) in Phase 0. Data Dictionary Protocol (DD0). Design Compliance Gate (DC0). Single Source of Truth rule. Post-mortem driven."
---
...
| R2-Physics | Physical plausibility, mechanism coherence, fundamental limits (Shannon, thermodynamics) | FORCED, BRAINSTORM | Searches literature for prior art, contradictions, known artifacts. Cites DOIs. |
...
### The Serendipity Principle (learned from real research experience)
...
In practice: an initial approach fails → Serendipity Engine flags an unexpected pattern → investigation pivots → months of adversarial testing → validated findings emerge. The serendipity flag was the BEGINNING, not the end.
...
## GATES (Complete List v5.5 — 36 gates)

### Pipeline Gates (G0-G6)
```
G0 (Input Sanity):     Data exists, format correct, no corruption
G1 (Schema):           Data schema/dataframe structure matches expectation
...
L-1 (Lit Pre-Check):   Prior art searched BEFORE committing to direction. NEW in v5.5.
                         IEEE Xplore + Optica + arXiv (physics.optics, eess.SP) for exact intersection + components.
                         Prior work → PIVOT or DIFFERENTIATE (explicit, documented).
L0 (Source Validity):   DOI verified, peer-reviewed status confirmed
...
> Contains: pass/fail criteria for all 36 gates (29 base + 4 HE + 7 v5.5 instruction-enforced; 8 schema-enforced), fail actions, gate tracking format.
...
3. → PHASE 0: SCIENTIFIC BRAINSTORM (mandatory, not skippable)
   ...
   3d. DATA: Reality check — does literature/data exist? (IEEE Xplore, Optica, SPIE, etc.)
...
- [ ] All claims have sources with DOI
...
| DATA | researcher | Literature audit via IEEE Xplore, Optica, etc. |
...
| **Literature discovery** | **openalex-database, web_search (IEEE Xplore, Optica, SPIE)** | **B0** |
| Literature search | openalex-database, perplexity-search, web_search skills | L0 |
| Data normalization | obs-normalizer (measurement data) | G0-G1 |
| Performance comparison | structured tables, statistical analysis | G2-G3 |
| Meta-analysis | statsmodels, literature-review skills | G4 |
| Visualization | scientific-visualization skill | G5, G6 |
| Standards lookup | web_search (ITU-T, IEEE 802.3) | varies |
...
| Obs Normalizer | `assets/obs-normalizer.md` | ACT-analyze (structured measurement data) |

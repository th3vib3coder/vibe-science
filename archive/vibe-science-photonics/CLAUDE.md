...
AI agents optimize for completion, not truth. They get excited by strong signals, construct narratives, don't search for what kills their claims, don't crystallize intermediate results, and declare "done" prematurely. This is not theoretical — it happened repeatedly in real research testing. The agent would have published completely confounded claims (strong statistical support that reversed when controlling for temperature and device generation) without the adversarial review architecture.
...
0. **EXPERT PRIORITY**: When the domain expert provides a knowledge assertion, it has priority over model speculation. Expert assertions are captured in EXPERT-ASSERTIONS.md with confidence floor 0.70. A claim that contradicts an expert assertion requires extraordinary evidence (E >= 0.85, peer-reviewed) to stand. The expert can override R2 on domain-specific grounds — the override is documented, not hidden.
...
3. **GATES BLOCK**: Quality gates are hard stops. 36 gates (8 schema-enforced). Fix first, re-gate, then continue.
...
- You MUST search (web, literature, IEEE Xplore, Optica, OpenAlex) for: prior art, contradictions, known artifacts, standard methodology.
- You MUST consult EXPERT-ASSERTIONS.md before every review — expert domain knowledge takes priority over model speculation.
...
### If you are the DOMAIN EXPERT (Human User):
- Your assertions have priority over model speculation (LAW 0).
- You can inject knowledge via Expert Knowledge Injection protocol (`protocols/expert-knowledge.md`).
- You can override R2's verdict on domain-specific grounds — the override is recorded, not hidden.
- You validate at Human Expert Gates (HE0-HE3) — the system STOPS for your review.
- Your expertise is in device physics, optics, electromagnetic theory — trust it.

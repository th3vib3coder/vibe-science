...
**When:** Before committing to any new research direction (Phase 0 Step 2b, or any mid-session pivot).
...
- Search not performed → HALT. No direction commitment without literature check.
- Queries not documented → Document queries and results, re-gate.
```
...
- Cross-check mismatch → HALT. Investigate alignment, column semantics, or computation.
- Perfect correlation with label → HALT. Likely data leakage. Investigate feature source.
- Unexpected sample/feature count → HALT. Investigate filtering, joins, or data loading.
```

### Gate DQ2 — Post-Training Model Quality
**When:** After training any predictive model. Before interpreting or using its outputs.
...
FAIL actions:
- Worse than baseline → HALT. Investigate feature quality (DQ1), data alignment, label correctness.
- Single feature dominance → WARN. Investigate whether that feature is a proxy for the label.
- Unstable folds → WARN. Investigate data heterogeneity, consider stratified splitting.
- Leakage indicators → HALT. Audit splitting strategy for information leakage.
```

### Gate DQ3 — Post-Calibration Statistical Quality
**When:** After any statistical calibration, validation, or uncertainty quantification step.
...
- Suspiciously perfect → HALT. Likely leakage or trivial solution (e.g., intervals so wide they cover everything).
- Inadequate sample size → WARN. Report precision honestly with appropriate significant figures.
```

### Gate DQ4 — Post-Finding Document Quality
**When:** After formulating any finding and writing it to FINDINGS.md or equivalent.
...
- No sample size → Add sample size before proceeding.
- No alternative explanations for surprising result → WARN. List at least one.
- Naming inconsistency → Fix all references to use consistent naming.
```
...
**When:** Before using any column/field from a dataset for the FIRST TIME in a session.
...
- Column meaning assumed from name alone → HALT. Verify from dataset documentation,
  README, or by inspecting values.
```
...
FAIL actions:
- Specified dataset not used → WARN. Either use it or document why it was dropped
  (decision logged with rationale).
- Method changed without documentation → HALT. Document the deviation and rationale.
- Design says X, execution does Y, no documentation → HALT. Either align execution
  with design or update design with documented rationale.
```
...
| L-1 | PASS | PubMed+bioRxiv searched, no prior art | queries documented |
| DQ1 | PASS | no zero-var, no leakage, cross-check OK | 23 features |
| DQ2 | PASS | AUC=0.85 > baseline 0.52, CV=0.12 | stable |
| DQ3 | PASS | coverage=0.952, n=1678 | plausible |
| DQ4 | PASS | all numbers match JSON, n reported | |
| DD0 | PASS | 15 columns documented | data-dictionary.md |
| DC0 | PASS | design aligned, all datasets used | |
```

# Reviewer 2 Ensemble Protocol v5.0
...
**v4.0 upgrades**: R2 as CO-PILOT (not just gate), confounder harness enforcement (LAW 9), escalating scrutiny (each pass MORE demanding), demolition-oriented web search, BRAINSTORM review mode, SHADOW mode (passive surveillance every 3 cycles), anti-premature-closure mandate.
...
The canonical R2 system prompt is defined in the agent instructions (section "R2 SYSTEM PROMPT"). This is the persona the agent adopts during CHECKPOINT-r2.
...
2. SFI INJECTION (orchestrator-level):
   ...
   → See: references/seeded-fault-injection.md
...
3. BLIND-FIRST PASS (Phase 1):
   ...
   → See: references/blind-first-pass.md
...
5. V0 GATE CHECK:
   ...
   → See: references/seeded-fault-injection.md, assets/schemas/vigilance-check.schema.json
...
Salvaged seeds are automatically written to SERENDIPITY.md with status `SALVAGED_FROM_R2` and enter the triage queue. They must conform to `assets/schemas/serendipity-seed.schema.json`.
...
See: `references/circuit-breaker.md`

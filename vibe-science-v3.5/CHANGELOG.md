...
## [3.5.1] — 2026-02-07 — TERTIUM DATUR (Bugfix + Polish)

### Fixed
- **SKILL.md**: Announce banner said "v3.0" → corrected to "v3.5"
- **evidence-engine.md**: Header still referenced "Nullis Secundus" → updated to "v3.5 — TERTIUM DATUR"
- **analysis-orchestrator.md**: Header still referenced "Nullis Secundus" → updated
- **audit-reproducibility.md**: Header still referenced "Nullis Secundus" → updated
- **skill-router.md**: Wrong file references (`reviewer2-v3.md` → `protocols/reviewer2-ensemble.md`, `assets/gates.md` → `gates/gates.md`)
- **templates.md**: Wrong file references (`references/evidence-engine.md` → `protocols/evidence-engine.md`, `references/reviewer2-v3.md` → `protocols/reviewer2-ensemble.md`)
- **serendipity-engine.md**: "10 minutes" time reference → replaced with quantitative objective
- **loop-otae.md**: "2-5 minutes / 10 minutes" time references → replaced with action-based guidance

### Added
- **D0-D2 Decision Gates** in `gates/gates.md`: D0 (Decision Justification), D1 (Claim Promotion), D2 (RQ Conclusion) — previously referenced but never defined
- **Evidence Floor Gate** in confidence formula: if E < 0.2, confidence capped at 0.20 — enforces LAW 2 at formula level (prevents sourceless claims reaching medium confidence)
- **Version tracking**: `vibe_science_version` field in STATE.md template, version compatibility check in resume protocol
- Removed duplicate root-level files (SKILL.md, README.md, CHANGELOG.md) — clean single-directory structure

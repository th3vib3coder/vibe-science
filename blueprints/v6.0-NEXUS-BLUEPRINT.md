...
async function setup() {
    // 1. Verifica/installa Bun (runtime per worker)
    if (!await commandExists('bun')) {
        await installBun();
    }

    // 2. Crea directory globali
    ...
    await fs.mkdir(path.join(globalDir, 'embeddings'), { recursive: true });

    // 3. Inizializza database SQLite
    const db = new Database(path.join(globalDir, 'db', 'vibe-science.db'));
    await runMigrations(db, path.join(__dirname, '..', 'db', 'migrations'));

    // 4. Carica sqlite-vec extension
    db.loadExtension('sqlite-vec');

    // 5. Avvia worker embedding (daemon Bun, leggero)
    await startEmbedWorker(globalDir);

    return { status: 'ready', db_path: db.filename };
}
```

**Dipendenze totali:** Bun (~30 MB, auto-install), sqlite-vec (~1 MB, bundled). Nessuna dipendenza Python.
...
/plugin marketplace add carminoski/vibe-science
...
```
setup.js execution:
  1. [Verifica Node.js >= 18]                          ✓ (pre-requisito Claude Code)
  2. [Verifica/Installa Bun]                            ~30 MB, auto-install se mancante
  3. [Crea ~/.vibe-science/]                            Directory globale
  4. [Crea ~/.vibe-science/db/vibe-science.db]          SQLite database
  5. [Esegui schema.sql + migrations]                   Tabelle + indici
  6. [Carica sqlite-vec extension]                      ~1 MB, bundled
  7. [Avvia worker-embed.js]                            Daemon Bun per embedding
  8. [Scarica all-MiniLM-L6-v2 model]                  ~90 MB, cached in ~/.vibe-science/embeddings/
  9. [Copia skill/ in progetto se non esiste]           Prompt files

Tempo totale prima installazione: ~30 secondi (prevalentemente download modello embedding)
Tempo su avvii successivi: <1 secondo (skip se tutto presente)
```
...
|-----------|-----------|---------------|---------|
| **Bun** | ~30 MB | Auto-install | Runtime per worker embedding |
| **sqlite-vec** | ~1 MB | Bundled nel plugin | Ricerca vettoriale in SQLite |
| **all-MiniLM-L6-v2** | ~90 MB | Download una volta | Embedding per ricerca semantica |
| **SQLite** | 0 (bundled) | Gia' in Bun/Node | Database strutturato |

**Totale: ~120 MB** (vs ~200 MB solo per Chroma + sentence-transformers di v5.5)

**Nessuna dipendenza Python.** Tutto il sistema gira su Node.js/Bun. Elimina il problema di gestire environment Python separati.

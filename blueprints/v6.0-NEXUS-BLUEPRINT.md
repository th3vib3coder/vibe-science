...
- Le **12 Leggi Immutabili** restano identiche
- L'architettura **OTAE-Tree** resta il cuore del loop
- **R2 Ensemble** (4 reviewer), **SFI**, **BFP**, **R3 Judge** restano
- Tutti i **32 gate** (25 v5.0 + 7 v5.5) restano — enforcement diventa reale
...
| v5.5 | ORO | Skill | DQ Gates, Research Spine, VibeBrAIn, SSOT, Silent Observer, R2 INLINE | 32 | 12 |
| **v6.0** | **NEXUS** | **Plugin** | **Infrastructure enforcement, persistent memory, auto-calibration, TEAM permissions** | **32** | **12** |
...
- Architecture: 7 lifecycle hooks + worker service (Express, port 37777) + SQLite + ChromaDB + SDK Agent per compressione
...
│   ├── hooks/
│   │   └── hooks.json                  # Hook definitions (7 lifecycle)
│   ├── scripts/
...
│   ├── gates/
│   │   └── gates.md                    # 32 gate (25 v5.0 + 7 v5.5)
...
CRYSTALLIZE                         Stop hook
  ...
  ├── STATE.md tree section          ← Stop: state export
...
**Invariante LAW 7:** Il sistema DEVE essere resumabile da `STATE.md` ANCHE senza il database SQLite. Il DB arricchisce il contesto (memorie, calibration, claim history) ma non e' indispensabile per il resume base. Fallback graceful.
...
function generateR2Hint(weaknesses, sfiStats, j0Trend) {
    ...
    if (topW && topW[1] >= 2) {
        hints.push(`R2 historically weak on "${topW[0]}" (decay-weighted score: ${topW[1].toFixed(1)}). High priority.`);
    }

    // SFI-based hints
    if (sfiStats.length > 0) {
        hints.push(`SFI: R2 most frequently misses fault type "${sfiStats[0].fault_type}". ` +
...
function buildContext(db, projectPath, sessionId) {
    ...
    context.state = lastSession
        ? `Last session: ${lastSession.total_actions} actions, ` +
          `${lastSession.claims_created} claims created, ${lastSession.claims_killed} killed.\n` +
...
| W9 | 12 Leggi Immutabili | **INVARIATO** — testo identico |
...
| Gate totali | 27 (8 schema) | 32 (8 schema) | **32** (8 schema, **enforced via code**) |
...
- **A1.** Tutti i 32 gate (25 v5.0 + 7 v5.5) presenti e funzionanti?
- **A2.** I 12 schema JSON invariati e read-only?
- **A3.** Le 12 Leggi Immutabili invariate? (testo identico)
...
- **B2.** hooks.json con 7 lifecycle hooks?
...
- **G5.** skill/gates/gates.md — 32 gate elencati?
...
|-------------------|------------------|-------------------|
| 7 lifecycle hooks | 7 lifecycle hooks | Identica struttura (Setup, SessionStart, UserPromptSubmit, PostToolUse, Stop) |

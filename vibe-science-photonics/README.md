...
**v5.5: R2 is structurally unbypassable. Not just prompted — architecturally enforced.**

[![DOI](https://zenodo.org/badge/1148022920.svg)](https://doi.org/10.5281/zenodo.18665031)
[![Version](https://img.shields.io/badge/version-5.5.0-blue.svg)](README.md#version-history)
...
[![Gates](https://img.shields.io/badge/quality_gates-36-red.svg)](#quality-gates)
...
║  │Injection│    │ Pass    │    │ 7 modes   │    │ RMS≥0.80 │       ║
║  └─────────┘    └─────────┘    └───────────┘    └────┬─────┘       ║
...
**What actually happened (21 sprints, photonics):**
- OR = 2.30 → **reversed sign** under propensity matching
- "Bidirectional effects" → **physically impossible**
- "Regime switch" → Cohen's d = **0.07** (noise)
- "Generalizable rankings" → **don't generalize** between measurement setups
...
**7 modes:** BRAINSTORM · FORCED · BATCH · SHADOW · VETO · REDIRECT · INLINE
...
> *"Observe, Recall, Operate."* Post-mortem driven. 7 new gates. R2 INLINE at every finding.

v5.5 is an **additive upgrade** born from real research post-mortems. Every change traces to a concrete mistake that was NOT caught by v5.0's automated checks. No new subsystems — only targeted improvements to existing protocols.
...
**7/7 = PASS | 5-6 = WARN | <5 = BLOCKED | Item 1 fail = HALT**

### Other v5.5 Changes

- **SSOT rule**: All numbers originate from structured data files. Documents reference, never define.
- **Structured logbook**: Mandatory LOGBOOK.md entry in CRYSTALLIZE for every OTAE cycle.
- **Operational integrity checks**: Orphaned datasets, document sync, design drift — checked in OBSERVE phase.

### What v5.5 Does NOT Change

The OTAE-Tree loop, 10 Constitutional Laws, R2 Ensemble architecture (R2-Physics, not R2-Bio), SFI, BFP, R3 Judge, Schema-Validated Gates, Circuit Breaker, Agent Permission Model, Serendipity Engine, 5-stage Experiment Manager, Expert Knowledge Injection, Human Expert Gates (HE0-HE3), and the separation of powers between Builder and Destroyer — all unchanged.
...
| **3** | GATES BLOCK | Quality gates are hard stops, not suggestions | 36 gates block pipeline (8 schema-enforced) |
| **4** | R2 IS CO-PILOT | R2 can VETO, REDIRECT, FORCE. Non-negotiable. | 7 activation modes |
...
**What it should do:** Four specialist reviewers (Methods, Statistics, Physics, Engineering) independently assess claims, then their verdicts are aggregated. Independence is the entire point — correlated reviewers add no value.
...
```bash
git clone https://github.com/vibe-science-photonics/vibe-science.git
...
| **Collision-Zone Thinking** | brainstorm-engine.md | Force cross-domain hypothesis (photonics × materials science, optics × thermodynamics) |
...
| Reviewer 2 Ensemble | `protocols/reviewer2-ensemble.md` | 4-domain adversarial review, 7 modes, 12 red flags, double-pass, circuit breaker integration |
...
| Data Extraction | `protocols/data-extraction.md` | NO TRUNCATION, structured measurement data schema, supplementary materials |
...
│   ├── reviewer2-ensemble.md       # R2: 7 modes, 4 reviewers, red flags
...
| 5.0 | IUDEX | 2026 | SFI, Judge Agent (R3), Blind-First Pass, Schema-Validated Gates, Circuit Breaker, Agent Permission Model. 29 gates (8 schema-enforced). R2 structurally unbypassable. |
| **5.5** | **ORO** | **2026** | **Post-mortem driven. 7 new gates (L-1, DQ1-DQ4, DD0, DC0). R2 INLINE mode. SSOT rule. Structured logbook. 36 total gates. Domain-general data quality enforcement.** |
...
graph TD
    ...
    AB[Anthropic scientific-research<br>toolkit] --> V2
    ...
    V50 --> V55[v5.5<br>ORO]
    PM[Post-mortem<br>research errors] --> V55

    style V55 fill:#ff6b6b,color:#fff,stroke:#333,stroke-width:3px
    style V50 fill:#4ecdc4,color:#fff
...
**Scientific toolkits**
*(Anthropic scientific-research toolkit, Claude Scientific Skills, MCP)*
...
Vibe Science is grounded in peer-reviewed research. Every architectural decision traces to an empirical finding:
...
| 2 | **Gou, Z., et al.** (2023). "CRITIC: LLMs Can Self-Correct with Tool-Interactive Critiquing." *ICLR 2024*. arXiv:2305.11738 | Self-correction works ONLY with external tool feedback. Model's own critiques "contribute marginally." | Validates R2's mandatory tool-use (IEEE Xplore, Scopus, OpenAlex). But prompt-level mandates can be circumvented — hence Schema-Validated Gates. |
...
| **Verification type** | Logical (reasoning-based) | **Empirical** (tool-grounded: IEEE Xplore, Scopus, web) |
...
**Complementary, not competing.** Deep Think catches what pure reasoning can catch. Vibe Science catches what only external empirical verification can catch. The critical architectural difference: **Vibe Science validates against external sources** (IEEE Xplore, OpenAlex, Scopus, web) rather than relying on internal self-correction. This makes it robust to the failure mode Huang et al. proved is fundamental.
...
- SFI fault taxonomy is domain-specific (photonics-derived) — needs customization for other domains
...
|----------|-------------------------------|
| **Physical lab execution** | Cannot run experiments; all evidence is computational or from published data |
...
| **Domain universality** | SFI taxonomy and gates tested on photonic device characterization; other domains need custom faults, gates, and MCP skills |
...
> Vibe Science Contributors (2026). *Vibe Science: an AI-native research engine with adversarial review and serendipity tracking.* DOI: [10.5281/zenodo.18665031](https://doi.org/10.5281/zenodo.18665031)
...
Copyright 2026 Vibe Science Contributors

---

**Authors**: Vibe Science Contributors

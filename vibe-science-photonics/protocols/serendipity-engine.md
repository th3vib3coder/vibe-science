...
3. **VALIDATION**: The anomaly survives confounder harness (LAW 9), cross-configuration replication, permutation testing, and R2 demolition. Only THEN is it a finding.

**The lesson from photonics research:** Initial device model failed to predict measured response (Sprint 3) → Serendipity Engine scored 13/15 → investigation pivoted to unexpected thermal crosstalk mechanism → 21 sprints of adversarial testing → 4 validated findings across multiple device configurations. The serendipity flag at Sprint 3 was the BEGINNING, not the end. Without the 18 subsequent sprints of falsification, the flag would have been meaningless.
...
## Expert-Originated Serendipity (EXPERT source type)

Seeds with `source: EXPERT_CONTRADICTION` represent cases where the domain expert's knowledge contradicts the literature or the system's findings. These are treated as high-priority signals:

- **Score minimum:** 10 (automatically QUEUE or higher — never dismissed as NOISE)
- **Contradiction priority:** When literature contradicts expert knowledge, this is the highest-priority serendipity signal (score >= 15, automatically INTERRUPT-eligible)
- **Rationale:** The expert's tacit knowledge often captures device behavior, fabrication realities, and measurement subtleties that are not published. When this contradicts published results, it usually indicates either (a) a genuine gap in the literature, (b) unreported experimental conditions, or (c) evolving technology that has outpaced publications.

### Periodic Expert Question

Every 10 cycles, the system MUST ask the domain expert:

> **"Have you noticed anything unexpected in the results so far?"**

The expert's response is processed as follows:
1. If the expert reports something unexpected → create a seed with `source: EXPERT_CONTRADICTION`, minimum score 10
2. If the expert says nothing unexpected → log as `EXPERT_CHECK: no new signals (cycle N)`
3. If the expert identifies a contradiction with the literature → score >= 15, tag as INTERRUPT
...
In v5.0, free-text seed entries are replaced by **structured, schema-validated seeds**. Every serendipity seed MUST conform to the schema defined in `schemas/serendipity-seed.schema.json`. This ensures that every seed is machine-parseable, traceable, and carries enough information for automated triage and deadline enforcement.
...
source: SCANNER | SALVAGED_FROM_R2 | CROSS_BRANCH | USER | EXPERT_CONTRADICTION
score: 0-20
...
- **`causal_question`** forces the seed author to articulate a mechanism, not just an observation. "Device X has anomalous insertion loss" is not a seed; "Device X's anomalous insertion loss is caused by mode mismatch at the waveguide taper junction due to fabrication underetch" is.

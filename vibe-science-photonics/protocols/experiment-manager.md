...
- Cross-configuration validation (diverse data rates, temperatures, fiber types) if generalizable claims are made
...
- Cross-configuration validation (diverse data rates, temperatures, fiber types) attempted (if generalizable claims)
...
## Design Compliance Check — Gate DC0 (v5.5)

At every stage transition AND at any point where the researcher realizes execution has diverged from the design, perform the DC0 check:
...
3. List: "What methods does the design specify?"
   → Are we using the specified methods? If not, why?
4. List: "What analyses were planned?"
   → Have all planned analyses been attempted or explicitly deferred?
5. If ANY deviation exists:
   → Document the deviation with rationale
   → Log as DEC-YYYYMMDD-NNN in decision-log.md
   → If deviation is significant (e.g., dropping a whole dataset or measurement condition): WARN
```

**Why this exists:** Post-mortem analysis revealed that a dataset can be downloaded early in a session but not used for hours, despite the design calling for it. DC0 catches this kind of execution drift early.
...
2b. Check design compliance gate DC0 (v5.5)
3. If PASS:
...
| Cross-configuration fails in Stage 4 | 4 | Downgrade generalizable claims, don't kill validated local claims |

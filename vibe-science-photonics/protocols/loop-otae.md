...
After reading state files but BEFORE reasoning, perform these checks:

1. **Orphaned datasets** — Are there downloaded data files not referenced in any script or analysis? If yes → WARN in PROGRESS.md. Likely a design compliance issue (DC0).
2. **Document sync** — If `results.json` (or equivalent structured data) was updated more recently than `FINDINGS.md`, the documents may be out of sync. If yes → flag for DQ4 check at next finding formulation.
3. **Design drift** — Compare current planned action with the research design (RQ.md). Are we doing what the design says? If not → either realign or document the deviation via DC0.

These checks are SYNCHRONOUS — run them as part of OBSERVE, not as a background process. They take seconds and prevent hours of wasted work.
...
- Any operational integrity issues from v5.5 checks above
...
### Data Quality Gates (v5.5) — inline with ACT

After the action executes, apply the relevant DQ gate BEFORE proceeding to EVALUATE:

| After this action... | Apply this gate |
|---------------------|----------------|
| Feature extraction from any dataset | **DQ1** (Post-Extraction) + **DD0** (Data Dictionary, if new dataset) |
| Model training | **DQ2** (Post-Training) |
| Statistical calibration / validation | **DQ3** (Post-Calibration) |
| Finding formulation | **DQ4** (Post-Finding) + **R2 INLINE** (7-point checklist) |

DQ gates are defined in `gates/gates.md`. They are BLOCKING: a DQ HALT stops the cycle. Fix the issue, re-run the gate, then continue.

**Key principle**: DQ gates verify data and operational quality. Existing v5.0 gates (G0-G6, L0-L2, etc.) verify claim and artifact quality. Both layers operate — they are complementary, not redundant.
...
5. **R2 INLINE check** (v5.5): Every finding passes the 7-point checklist BEFORE recording in CLAIM-LEDGER. See `reviewer2-ensemble.md` INLINE mode. If INLINE blocks → fix before recording.
6. **Confidence scoring**: Apply formula (see evidence-engine.md) — computed, NOT felt
7. **Assumption check**: Did any assumption change? → ASSUMPTION-REGISTER.md
8. **Metric parsing** (if computational node): Extract metrics, compute delta from parent
9. **VLM feedback** (if figures generated and VLM available): Apply Gate G6
10. **Document sync check** (v5.5): If this result updates numbers in a structured file (JSON, CSV), verify that ALL documents referencing those numbers are consistent. Apply Gate DQ4 if a finding was formulated.
...
- G0: Data loaded correctly? (units verified, no NaN/Inf, ranges physical)
- G1: Measurement schema normalized?
- G2: Design justified? (comparison variables, controlled confounders, normalization choices)
- G3: Analysis converged? (results stable, no numerical artifacts)
- G4: Metrics meaningful? (BER, power penalty, reach contextualized, trade-offs explicit)
...
- **Output:** [files/data produced, key metrics]
- **Gate status:** [which gates applied, PASS/WARN/HALT]
